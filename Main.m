...
if data_flag == 1
    ...
    if rank(Ab)~= size(Ahat,1)
        ...
        if isequal(sort(correct_edges),1:e_c) 
            fprintf(2, 'A graph is exactly identified from given data in %f seconds and its incidence matrix is as follows \n', time);
            Inc_pred
        else
            pred_edges = setdiff(correct_edges,branch);
            fprintf(2, 'Among the unknown edges connectivities, only %d could possibly be identified from given data', size(pred_edges,2));
        end
    end
else
    if noise_flag == 0
        ...
        for i=1:Nrepeats
            ...
            if size(Ahat,1)==b    % Testing if model order is correctly identified
                ...
                if isequal(Inc_pred,Inc_Con)
                    ...
                    fprintf(2, 'With dataset %d, only %0.2f percent of the unknown edge connections could be identified correctly \n', i, test1(i));
                end
            end
            time(i) = toc(tstart);
        end
...
%         avg_test1 = sum(test1)/Nrepeats %%%
        if avg_test==0
            fprintf(2, 'The conservation graph could not be completely identified from any of %d dataset(s) that were generated \n', Nrepeats);
        else
